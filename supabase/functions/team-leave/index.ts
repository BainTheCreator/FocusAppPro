...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    const mem=await admin.from('team_members').select('id,role').eq('team_id',team_id).eq('user_id',user_id).maybeSingle();
    if(mem.error||!mem.data) return json({error:'NOT_MEMBER'},400);

    if(mem.data.role==='owner'){
      const others=await admin.from('team_members').select('id').eq('team_id',team_id).neq('user_id',user_id);
      if((others.data||[]).length>0){
        return json({error:'OWNER_CANNOT_LEAVE_WITH_MEMBERS'},400);
      }
    }

    const del=await admin.from('team_members').delete().eq('team_id',team_id).eq('user_id',user_id);
